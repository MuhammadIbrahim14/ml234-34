...
export default function Auth({ mode = 'login' }) {
  const { signIn, signUp, isConfigured, authError, setAuthError } = useAuth();
  const [isFarmer, setIsFarmer] = useState(false);
  const [demoRole, setDemoRole] = useState(ROLES.CUSTOMER);
  ...
  async function onSubmit(e) {
    ...
    // Live auth uses profile.role from DB; demo mode uses explicit portal selection
  ...
  return (
    <div className="auth-page">
      ...
      <div className="auth-card">
        ...
        <h1>{mode === 'login' ? 'Login to MarketLink' : 'Join the local food community'}</h1>
        <p>
          {isConfigured
            ? 'Secure sign-in powered by Supabase Auth. Role dashboards for customers, farmers, and admins.'
        ...
        <form onSubmit={onSubmit}>
          ...
          {mode === 'login' && !isConfigured && (
            <div className="role-toggle">
              <button type="button" className={demoRole === ROLES.CUSTOMER ? 'selected' : ''} onClick={() => setDemoRole(ROLES.CUSTOMER)}>
                Customer
              </button>
